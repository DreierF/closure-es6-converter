...
goog.scope(function() {
	goog.net.XhrIo = class extends goog.events.EventTarget {
		/**
		...
		constructor(opt_xmlHttpFactory) {
			 ...
			 * @const
			 */
			this.logger_ = goog.log.getLogger('goog.net.XhrIo');
		...
		send(url, opt_method, opt_content, opt_headers) {
			if (this.xhr_) {
				throw new Error(
					'[goog.net.XhrIo] Object is active with another request=' + this.lastUri_ + '; newUri=' + url);
			}
			...
			if (goog.array.contains(goog.net.XhrIo.METHODS_WITH_FORM_DATA, method) && !contentTypeKey
				&& !contentIsFormData) {
			...
			try {
				...
				if (this.timeoutInterval_ > 0) {
					...
					goog.log.fine(this.logger_, this.formatMsg_(
						'Will abort after ' + this.timeoutInterval_ + 'ms if incomplete, xhr2 '
		...
		onReadyStateChangeHelper_() {
			...
			} else if (this.xhrOptions_[goog.net.XmlHttp.OptionType.LOCAL_REQUEST_ERROR] && this.getReadyState()
		...
		onProgressHandler_(e, opt_isDownload) {
			...
			this.dispatchEvent(goog.net.XhrIo.buildProgressEvent_(e,
		...
		cleanUpXhr_(opt_fromDispose) {
			if (this.xhr_) {
				...
				var xhr = this.xhr_;
				var clearedOnReadyStateChange = this.xhrOptions_[goog.net.XmlHttp.OptionType.USE_NULL_FUNCTION]
		 ...
		 */
		getReadyState() {
			return this.xhr_ ? /** @type {goog.net.XmlHttp.ReadyState} */ (this.xhr_.readyState)
		 ...
		 */
		static send(url, opt_callback, opt_method, opt_content, opt_headers, opt_timeoutInterval, opt_withCredentials) {
		...
		static protectEntryPoints(errorHandler) {
			goog.net.XhrIo.prototype.onReadyStateChangeEntryPoint_ =
				errorHandler.protectEntryPoint(goog.net.XhrIo.prototype.onReadyStateChangeEntryPoint_);
		}
		...
		static shouldUseXhr2Timeout_(xhr) {
			return goog.userAgent.IE && goog.userAgent.isVersionOrHigher(9) && goog.isNumber(
				xhr[goog.net.XhrIo.XHR2_TIMEOUT_]) && goog.isDef(xhr[goog.net.XhrIo.XHR2_ON_TIMEOUT_]);
		}
		 ...
		 * @private
		 */
		static isContentTypeHeader_(header) {
		 ...
		 * @private
		 */
		static buildProgressEvent_(e, eventType) {
			return /** @type {!ProgressEvent} */ ({
				type: eventType, lengthComputable: e.lengthComputable, loaded: e.loaded, total: e.total
			});
		}
	};
	...
	goog.net.XhrIo.ResponseType = {
		DEFAULT: '', TEXT: 'text', DOCUMENT: 'document', // Not supported as of Chrome 10.0.612.1 dev
		BLOB: 'blob', ARRAY_BUFFER: 'arraybuffer'
	};
