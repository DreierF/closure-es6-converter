...
goog.Promise = class {
  ...
  then(opt_onFulfilled, opt_onRejected, opt_context) {
    ...
    return this.addChildPromise_(goog.isFunction(opt_onFulfilled) ? opt_onFulfilled : null,
   ...
   * @template THIS
   */
  thenVoid(opt_onFulfilled, opt_onRejected, opt_context) {
    ...
    this.addCallbackEntry_(
        goog.Promise.getCallbackEntry_(opt_onFulfilled || goog.nullFunction, opt_onRejected || null, opt_context));
  }
  ...
  appendLongStack_(err) {
    if (goog.Promise.LONG_STACK_TRACES && err && goog.isString(err.stack) && this.stack_.length) {
      ...
      for (var promise = this; promise; promise = promise.parent_) {
        ...
        longTrace.push(
            'Value: ' + '[' + (promise.state_ == goog.Promise.State_.REJECTED ? 'REJECTED' : 'FULFILLED') + '] ' + '<' + String(promise.result_) + '>');
      }
      err.stack += '\n\n' + longTrace.join('\n');
    }
  }
  ...
  static allSettled(promises) {
    return new goog.Promise(function(resolve, reject) {
      ...
      for (var i = 0, promise; i < promises.length; i++) {
        promise = promises[i];
        goog.Promise.resolveThen_(promise, goog.partial(onSettled, i, true /* fulfilled */),
   ...
   * @private
   */
  static tryThen_(thenable, then, onFulfilled, onRejected, context) {
...
/**
   ...
   */
goog.Promise.CallbackEntry_ = class {
  /**
  * Entries in the callback chain. Each call to `then`,
  * `thenCatch`, or `thenAlways` creates an entry containing the
  * functions that may be invoked once the Promise is settled.
...
/**
   ...
   * @final
   */
goog.Promise.CancellationError = class extends goog.debug.Error {
  /**
  * Error used as a rejection reason for canceled Promises.
  *
