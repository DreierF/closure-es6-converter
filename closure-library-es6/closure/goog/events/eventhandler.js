  ...
  *
  ...
  listen_(src, type, opt_fn, opt_options, opt_scope) {
    ...
    for (var i = 0; i < type.length; i++) {
      var listenerObj = goog.events.listen(src, type[i], opt_fn || self.handleEvent, opt_options || false,
  ...
  listenOnce_(src, type, opt_fn, opt_options, opt_scope) {
    ...
    if (goog.isArray(type)) {
      ...
      var listenerObj = goog.events.listenOnce(src, type, opt_fn || self.handleEvent, opt_options,
          opt_scope || self.handler_ || self);
      if (!listenerObj) {
        // When goog.events.listen run on OFF_AND_FAIL or OFF_AND_SILENT
        // (goog.events.CaptureSimulationMode) in IE8-, it will return null
  ...
  unlisten(src, type, opt_fn, opt_options, opt_scope) {
    ...
    if (goog.isArray(type)) {
      ...
      var listener = goog.events.getListener(src, type, opt_fn || self.handleEvent, capture,
          opt_scope || self.handler_ || self);
