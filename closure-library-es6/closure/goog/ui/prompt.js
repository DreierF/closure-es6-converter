...
/**
   ...
   * 
   */
goog.ui.Prompt = class extends goog.ui.Dialog {
  /**
  * Creates an object that represents a prompt (used in place of javascript's
  * prompt). The html structure of the prompt is the same as the layout for
  * dialog.js except for the addition of a text box which is placed inside the
  ...
  *
   */
  constructor(promptTitle, promptBody, callback, opt_defaultValue, opt_class, opt_useIframeForIE, opt_domHelper) {
    ...
    var label = goog.html.SafeHtml.create('label', {'for': this.inputElementId_}, goog.html.SafeHtml.htmlEscapePreservingNewlines(promptBody));
     ...
     * @private
     */
    this.validationFn_ = goog.functions.TRUE;
  }
  ...
  createDom() {
    ...
    if (this.rows_ == 1) {
      ...
      this.userInputEl_ = this.getDomHelper().createDom(goog.dom.TagName.INPUT, {
        'className': goog.getCssName(cls, 'userInput'), 'value': this.defaultValue_
      });
    ...
    contentEl.appendChild(this.getDomHelper().createDom(goog.dom.TagName.DIV, {'style': 'overflow: auto'}, this.userInputEl_));
  }
