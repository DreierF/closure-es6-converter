...
goog.ui.MenuItem = class extends goog.ui.Control {
  /**
  ...
  constructor(content, opt_model, opt_domHelper, opt_renderer) {
    super( content, opt_renderer || goog.ui.MenuItemRenderer.getInstance(), opt_domHelper);
  ...
  getCaption() {
    ...
    if (goog.isArray(content)) {
      ...
      var caption = goog.array.map(content, function(node) {
        if (goog.dom.isElement(node) && (goog.dom.classlist.contains(/** @type {!Element} */ (node), acceleratorClass) || goog.dom.classlist.contains(
            /** @type {!Element} */ (node), mnemonicWrapClass))) {
          return '';
        } else {
  ...
  getAccelerator() {
    ...
    if (goog.isArray(content)) {
      var acceleratorEl = goog.array.find(content, function(e) {
        return goog.dom.classlist.contains(/** @type {!Element} */ (e), goog.ui.MenuItem.ACCELERATOR_CLASS);
      });
      if (acceleratorEl) {
   ...
   */
  getParent() {
    return /** @type {goog.ui.Menu} */ (super.getParent());
  }
  ...
  getParentEventTarget() {
    return /** @type {goog.ui.Menu} */ (super.getParentEventTarget());
  }
};
