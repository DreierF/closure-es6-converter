...
goog.ui.Dialog = class extends goog.ui.ModalPopup {
  /**
  ...
  createDom() {
    ...
    var dom = this.getDomHelper();
    this.titleEl_ = dom.createDom(goog.dom.TagName.DIV, goog.getCssName(this.class_, 'title'),
        this.titleTextEl_ = dom.createDom(goog.dom.TagName.SPAN, {
          'className': goog.getCssName(this.class_, 'title-text'), 'id': this.getId()
        }, this.title_),
        this.titleCloseEl_ = dom.createDom(goog.dom.TagName.SPAN, goog.getCssName(this.class_, 'title-close'))), goog.dom.append(
        element, this.titleEl_,
        this.contentEl_ = dom.createDom(goog.dom.TagName.DIV, goog.getCssName(this.class_, 'content')),
        this.buttonEl_ = dom.createDom(goog.dom.TagName.DIV, goog.getCssName(this.class_, 'buttons')));
  ...
  setDraggerLimits_(e) {
    ...
    if (goog.style.getComputedPosition(this.getElement()) == 'fixed') {
      ...
      this.dragger_.setLimits(
          new goog.math.Rect(0, 0, Math.max(0, viewSize.width - dialogSize.width), Math.max(0, viewSize.height - dialogSize.height)));
    } else {
  ...
  onKey_(e) {
    ...
    if (e.type == goog.events.EventType.KEYDOWN) {
      ...
      if (this.escapeToCancel_ && e.keyCode == goog.events.KeyCodes.ESC) {
        ...
        if (cancel && !isSpecialFormElement) {
          ...
          close = this.dispatchEvent(new goog.ui.Dialog.Event(cancel, /** @type {Element|null|string} */ (caption)));
    ...
    } else if (target == this.titleCloseEl_ && e.keyCode == goog.events.KeyCodes.SPACE) {
...
/**
   ...
   * 
   */
goog.ui.Dialog.Event = class extends goog.events.Event {
  /**
  * Dialog event class.
  ...
  *
  ...
  constructor(key, caption) {
    super(goog.ui.Dialog.EventType.SELECT);
...
/**
   ...
   */
goog.ui.Dialog.ButtonSet = class extends goog.structs.Map {
  /**
  * A button set defines the behaviour of a set of buttons that the dialog can
  * show.  Uses the {@link goog.structs.Map} interface.
  * @param {goog.dom.DomHelper=} opt_domHelper Optional DOM helper; see {@link
