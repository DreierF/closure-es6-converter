...
/**
   ...
   *     (Default: true).
   *
   (Default: 150). Use -1 to disable updates on typing. Note
   * 
   */
goog.ui.ac.InputHandler = class extends goog.Disposable {
  /**
  * Class for managing the interaction between an auto-complete object and a
  * text-input or textarea.
  *
  ...
  *
  ...
  constructor(opt_separators, opt_literals, opt_multi, opt_throttleTime) {
     ...
     * @private
     */
    this.updateDuringTyping_ = true;  // Initialize to a non-existent value.
  }
   ...
   */
  setSeparators(separators, opt_defaultSeparators) {
    this.separators_ = separators;
    this.defaultSeparator_ = goog.isDefAndNotNull(opt_defaultSeparators) ? opt_defaultSeparators : this.separators_.substring(0, 1);
   ...
   */
  setEndsWithSeparatorRegExp(separatorCheck) {
   ...
   */
  setPreventSelectionOnTab(newValue) {
