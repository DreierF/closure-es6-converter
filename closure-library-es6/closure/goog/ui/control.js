 ...
 * for example usage.
 */
goog.ui.Control = class extends goog.ui.Component {
	/**
	...
	constructor(opt_content, opt_renderer, opt_domHelper) {
		...
		this.supportedStates_ =
			goog.ui.Component.State.DISABLED | goog.ui.Component.State.HOVER | goog.ui.Component.State.ACTIVE
	...
	getCaption() {
		...
		var caption = goog.isString(content) ? content : goog.isArray(content) ? goog.array.map(content,
			goog.dom.getRawTextContent).join('') : goog.dom.getTextContent(/** @type {!Node} */ (content));
	...
	setVisible(visible, opt_force) {
		if (opt_force || (this.visible_ != visible && this.dispatchEvent(
	...
	setDispatchTransitionEvents(states, enable) {
		this.statesWithTransitionEvents_ =
			enable ? this.statesWithTransitionEvents_ | states : this.statesWithTransitionEvents_ & ~states;
	}
	...
	isTransitionAllowed(state, enable) {
		return this.isSupportedState(state) && this.hasState(state) != enable && (!(this.statesWithTransitionEvents_
			& state) || this.dispatchEvent(goog.ui.Component.getStateTransitionEvent(state, enable)))
			&& !this.isDisposed();
	}
	...
	handleMouseOver(e) {
		...
		if (!goog.ui.Control.isMouseEventWithinElement_(e, this.getElement()) && this.dispatchEvent(
			goog.ui.Component.EventType.ENTER) && this.isEnabled() && this.isAutoState(goog.ui.Component.State.HOVER)) {
	...
	performActionInternal(e) {
		...
		if (e) {
			...
			actionEvent = /** @type {!goog.events.BrowserEvent} */ (actionEvent);
 ...
 */
goog.ui.Control.getDecorator = /** @type {function(Element): goog.ui.Control} */ (goog.ui.registry.getDecorator);
...
goog.ui.Control.IeMouseEventSequenceSimulator_ = class extends goog.Disposable {
	/**
	...
	constructor(control) {
		...
		this.handler_.listen(element, goog.events.EventType.MOUSEDOWN, this.handleMouseDown_).
...
// the corresponding event listeners in order.
		...
		var down = goog.ui.Control.IeMouseEventSequenceSimulator_.makeLeftMouseEvent_(event,
...
// Restore original values for click handlers that have not yet been invoked.
	 ...
	 * @private
	 */
	static makeLeftMouseEvent_(e, typeArg) {
		'use strict';
		...
		event.initMouseEvent(typeArg, e.bubbles, e.cancelable, e.view || null,  // IE9 errors if view is undefined
			e.detail, e.screenX, e.screenY, e.clientX, e.clientY, e.ctrlKey, e.altKey, e.shiftKey, e.metaKey,
			goog.events.BrowserEvent.MouseButton.LEFT, e.relatedTarget || null);  // IE9 errors if relatedTarget is
																				  // undefined
		return event;
	}
};
