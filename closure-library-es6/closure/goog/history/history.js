...
goog.History = class extends goog.events.EventTarget {
  /**
   ...
   *     document.write.
   */
  constructor(opt_invisible, opt_blankPageUrl, opt_input, opt_iframe) {
    super();
    ...
    if (opt_input) {
      ...
      var inputHtml = goog.html.SafeHtml.create('input', {
        type: goog.dom.InputType.TEXT, name: inputId, id: inputId, style: goog.string.Const.from('display:none')
      });
    ...
    if (opt_invisible || goog.History.LEGACY_IE) {
      ...
      if (opt_iframe) {
        ...
        var iframeHtml = goog.html.SafeHtml.createIframe(this.iframeSrc_, null, {
          id: iframeId, style: goog.string.Const.from('display:none'), sandbox: undefined
        });
   ...
   * @private
   */
  setHistoryState_(token, replace, opt_title) {
  ...
  setHash_(token, opt_replace) {
    ...
    if (url != loc.href) {
      if (opt_replace) {
        ...
        goog.dom.safe.setLocationHref(loc,
            goog.html.uncheckedconversions.safeUrlFromStringKnownToSatisfyTypeContract(goog.string.Const.from('URL taken from location.href.'),
                url));
      }
    }
  }
   ...
   * @private
   */
  setIframeToken_(token, opt_replace, opt_title) {
    if (this.unsetIframe_ || token != this.getIframeToken_()) {
      ...
      if (goog.userAgent.IE) {
        ...
        var iframeSourceHtml = goog.html.SafeHtml.concat(goog.html.SafeHtml.create('title', {}, (opt_title || this.window_.document.title)),
            goog.html.SafeHtml.create('body', {}, token));
        goog.dom.safe.documentWrite(doc, iframeSourceHtml);
        doc.close();
      } else {
        goog.asserts.assertInstanceof(this.iframeSrc_, goog.html.TrustedResourceUrl,
            'this.iframeSrc_ must be set on calls to setIframeToken_');
        var url = goog.html.TrustedResourceUrl.unwrap(
            /** @type {!goog.html.TrustedResourceUrl} */ (this.iframeSrc_)) + '#' + token;
