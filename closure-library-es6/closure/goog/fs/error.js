 ...
 * @final
 */
goog.fs.Error = class extends goog.debug.Error {
  ...
  constructor(error, action) {
    ...
    if (goog.isDef(error.name)) {
      ...
      tempCode = goog.asserts.assertNumber(error.code);
      tempName = goog.fs.Error.getNameFromCode_(error.code);
    }

    super(goog.string.subs('%s %s', tempName, action));
