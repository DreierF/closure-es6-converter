...
goog.crypt.Aes = class {
   ...
   * @param {!Array<Array<number>>} state Current state.
   * @private
   */
  testStartRound_(roundNum, state) {}
   ...
   * @private
   */
  testAfterSubBytes_(roundNum, state) {}
   ...
   * @private
   */
  testAfterShiftRows_(roundNum, state) {}
   ...
   * @private
   */
  testAfterMixColumns_(roundNum, state) {}
   ...
   * @private
   */
  testKeySchedule_(roundNum, schedule, index) {}
  ...
  copyInput_(input) {
    ...
    for (var r = 0; r < goog.crypt.Aes.BLOCK_SIZE_IN_WORDS_; r++) {
      for (var c = 0; c < 4; c++) {
        ...
        goog.asserts.assert(v <= 255 && v >= 0, 'Invalid input. Value %s at position %s is not a byte.', v, p);
  ...
  invMixColumns_() {
    ...
    for (var c = 0; c < 4; c++) {
      ...
      s[0][c] = (goog.crypt.Aes.MULT_E_[t[0]] ^ goog.crypt.Aes.MULT_B_[t[1]] ^ goog.crypt.Aes.MULT_D_[t[2]] ^ goog.crypt.Aes.MULT_9_[t[3]]);

      s[1][c] = (goog.crypt.Aes.MULT_9_[t[0]] ^ goog.crypt.Aes.MULT_E_[t[1]] ^ goog.crypt.Aes.MULT_B_[t[2]] ^ goog.crypt.Aes.MULT_D_[t[3]]);

      s[2][c] = (goog.crypt.Aes.MULT_D_[t[0]] ^ goog.crypt.Aes.MULT_9_[t[1]] ^ goog.crypt.Aes.MULT_E_[t[2]] ^ goog.crypt.Aes.MULT_B_[t[3]]);

      s[3][c] = (goog.crypt.Aes.MULT_B_[t[0]] ^ goog.crypt.Aes.MULT_D_[t[1]] ^ goog.crypt.Aes.MULT_9_[t[2]] ^ goog.crypt.Aes.MULT_E_[t[3]]);
    }
  }
  ...
  keyExpansion_() {
    ...
    for (var rowNum = 0; rowNum < this.keyLengthInWords_; rowNum++) {
      this.keySchedule_[rowNum] = [this.key_[4 * rowNum], this.key_[4 * rowNum + 1], this.key_[4 * rowNum + 2], this.key_[4 * rowNum + 3]];
    }

    var temp = new Array(4);

    for (var rowNum = this.keyLengthInWords_; rowNum < (goog.crypt.Aes.BLOCK_SIZE_IN_WORDS_ * (this.numberOfRounds_ + 1));
         rowNum++) {
   ...
   * @private
   */
  static assertKeyArray_(arr) {
    if (goog.asserts.ENABLE_ASSERTS) {
      goog.asserts.assert(arr.length == 16 || arr.length == 24 || arr.length == 32,
