...
/**
   ...
   * @struct
   */
goog.fx.AbstractDragDrop = class extends goog.events.EventTarget {
  /**
  * Abstract class that provides reusable functionality for implementing drag
  * and drop functionality.
  *
  ...
  *
   ...
   * @protected
   */
  createDraggerFor(sourceEl, el, event) {
  ...
  endDrag(event) {
    ...
    if (activeTarget && activeTarget.target_) {
      ...
      var dragEvent = new goog.fx.DragDropEvent(goog.fx.AbstractDragDrop.EventType.DRAG, this, this.dragItem_,
          activeTarget.target_, activeTarget.item_, activeTarget.element_, clientX, clientY, x, y);
      this.dispatchEvent(dragEvent);

      var dropEvent = new goog.fx.DragDropEvent(goog.fx.AbstractDragDrop.EventType.DROP, this, this.dragItem_,
          activeTarget.target_, activeTarget.item_, activeTarget.element_, clientX, clientY, x, y, subtarget, event.browserEvent);
    ...
    var dragEndEvent = new goog.fx.DragDropEvent(goog.fx.AbstractDragDrop.EventType.DRAGEND, this, this.dragItem_,
        activeTarget ? activeTarget.target_ : undefined, activeTarget ? activeTarget.item_ : undefined,
  ...
  moveDrag_(event) {
    ...
    this.dispatchEvent(new goog.fx.DragDropEvent(goog.fx.AbstractDragDrop.EventType.DRAG, this, this.dragItem_,
        activeTarget ? activeTarget.target_ : undefined, activeTarget ? activeTarget.item_ : undefined,
        activeTarget ? activeTarget.element_ : undefined, event.clientX, event.clientY, x, y));
    ...
    if (activeTarget) {
      ...
      if (activeTarget.target_) {
        var sourceDragOutEvent = new goog.fx.DragDropEvent(goog.fx.AbstractDragDrop.EventType.DRAGOUT, this, this.dragItem_,
        ...
        var targetDragOutEvent = new goog.fx.DragDropEvent(goog.fx.AbstractDragDrop.EventType.DRAGOUT, this, this.dragItem_,
            activeTarget.target_, activeTarget.item_, activeTarget.element_, undefined, undefined, undefined, undefined, this.activeSubtarget_);
    ...
    if (this.targetBox_.contains(position)) {
      ...
      if (activeTarget && activeTarget.target_) {
        ...
        var sourceDragOverEvent = new goog.fx.DragDropEvent(goog.fx.AbstractDragDrop.EventType.DRAGOVER, this, this.dragItem_,
            activeTarget.target_, activeTarget.item_, activeTarget.element_);
        ...
        // target DragDrop can manage these events without having to know what
        // sources this is a target for.
        var targetDragOverEvent = new goog.fx.DragDropEvent(goog.fx.AbstractDragDrop.EventType.DRAGOVER, this, this.dragItem_,
            activeTarget.target_, activeTarget.item_, activeTarget.element_, event.clientX, event.clientY, undefined, undefined, subtarget);
   ...
   * @suppress {deprecated}
   */
  createDragElementInternal(sourceEl) {
  ...
  maybeCreateDummyTargetForPosition_(x, y) {
    ...
    for (var i = 0, target; target = this.targetList_[i]; i++) {
      ...
      if (target.scrollableContainer_) {
        ...
        box = new goog.math.Box(Math.max(box.top, scrollBox.top), Math.min(box.right, scrollBox.right),
            Math.min(box.bottom, scrollBox.bottom), Math.max(box.left, scrollBox.left));
      }
...
/**
   ...
   * @struct
   */
goog.fx.DragDropEvent = class extends goog.events.Event {
  ...
  *     that caused this dragdrop event.
   */
  constructor(type, source, sourceItem, opt_target, opt_targetItem, opt_targetElement, opt_clientX, opt_clientY, opt_x, opt_y, opt_subtarget, opt_browserEvent) {
 ...
 * @struct
 */
goog.fx.DragDropItem = class extends goog.events.EventTarget {
  /**
  ...
  maybeStartDrag_(event, element) {
    ...
    this.eventHandler_.listen(element, eventType.MOUSEMOVE, this.mouseMove_, false).listen(element, eventType.MOUSEOUT, this.mouseMove_, false);
  ...
  mouseMove_(event) {
    ...
    var mouseOutOnDragElement = event.type == goog.events.EventType.MOUSEOUT && event.target == currentDragElement;
...
/**
   ...
   * @private
   */
goog.fx.ActiveDropTarget_ = class {
  /**
  * Class representing an active drop target
  *
...
/**
   ...
   * @private
   */
goog.fx.ScrollableContainer_ = class {
