...
/**
   ...
   * @template T
   */
goog.structs.InversionMap = class {
  /**
  * Maps ranges to values.
  ...
  *
   ...
   * @private
   */
  storeInversion_(rangeArray, opt_delta) {
  ...
  spliceInversion(rangeArray, valueArray, opt_delta) {
    ...
    this.rangeArray = this.rangeArray.slice(0, startSplice).concat(otherMap.rangeArray).concat(this.rangeArray.slice(endSplice + 1));
    this.values = this.values.slice(0, startSplice).concat(otherMap.values).concat(this.values.slice(endSplice + 1));
  }
