...
goog.soy.Renderer = class {
  ...
  *
  ...
  renderAsFragment(template, opt_templateData) {
    var node = goog.soy.renderAsFragment(template, opt_templateData, this.getInjectedData_(), this.dom_);
  ...
  renderAsElement(template, opt_templateData) {
    var element = goog.soy.renderAsElement(template, opt_templateData, this.getInjectedData_(), this.dom_);
  ...
  renderElement(element, template, opt_templateData) {
    goog.soy.renderElement(element, template, opt_templateData, this.getInjectedData_());
    this.handleRender(element);
  }
  ...
  render(template, opt_templateData) {
    var result = template(opt_templateData || {}, undefined, this.getInjectedData_());
    goog.asserts.assert(!(result instanceof goog.soy.data.SanitizedContent) || result.contentKind === goog.soy.data.SanitizedContentKind.HTML,
        'render was called with a strict template of kind other than "html"' + ' (consider using renderText or renderStrict)');
    this.handleRender();
  ...
  renderText(template, opt_templateData) {
    ...
    goog.asserts.assert(result.contentKind === goog.soy.data.SanitizedContentKind.TEXT,
   ...
   * @template ARG_TYPES
   */
  renderStrict(template, opt_templateData) {
    return this.renderStrictOfKind(template, opt_templateData, goog.soy.data.SanitizedContentKind.HTML);
  }
   ...
   * @template ARG_TYPES
   */
  renderStrictUri(template, opt_templateData) {
    return this.renderStrictOfKind(template, opt_templateData, goog.soy.data.SanitizedContentKind.URI);
  }
   ...
   */
  renderStrictOfKind(template, opt_templateData, opt_kind) {
    var result = template(opt_templateData || {}, undefined, this.getInjectedData_());
    goog.asserts.assertInstanceof(result, goog.soy.data.SanitizedContent,
        'renderStrict cannot be called on a non-strict soy template');
    goog.asserts.assert(result.contentKind === (opt_kind || goog.soy.data.SanitizedContentKind.HTML),
   ...
   * @template ARG_TYPES
   */
  renderSafeHtml(template, opt_templateData) {
   ...
   * @template ARG_TYPES
   */
  renderSafeStyleSheet(template, opt_templateData) {
    var result = this.renderStrictOfKind(template, opt_templateData, goog.soy.data.SanitizedContentKind.CSS);
 ...
 * @interface
 */
goog.soy.InjectedDataSupplier = class {
