 ...
 * @final
 */
goog.html.SafeHtmlFormatter = class {
  /**
  ...
  format(format) {
    ...
    var html = goog.string.htmlEscape(format).replace(new RegExp('\\{' + marker + '[\\w&#;]+\\}', 'g'),
        goog.bind(this.replaceFormattingString_, this, openedTags));
    goog.asserts.assert(openedTags.length == 0, 'Expected no unclosed tags, got <' + openedTags.join('>, <') + '>.');
   ...
   */
  startTag(tagName, opt_attributes) {
    goog.html.SafeHtml.verifyTagName(tagName);
    return this.storeReplacement_({
      startTag: tagName, attributes: goog.html.SafeHtml.stringifyAttributes(tagName, opt_attributes)
    });
  }
  ...
  storeReplacement_(replacement) {
    ...
    var marker = '{' + goog.html.SafeHtmlFormatter.MARKER_ + this.replacementsCount_ + '_' + goog.string.getRandomString() + '}';
