 ...
 * @final
 */
goog.i18n.DateTimeFormat = class {
  /**
  ...
  constructor(pattern, opt_dateTimeSymbols) {
    ...
    goog.asserts.assert(goog.isDef(opt_dateTimeSymbols) || goog.isDef(goog.i18n.DateTimeSymbols),
     ...
     */
    this.dateTimeSymbols_ = /** @type {!goog.i18n.DateTimeSymbolsType} */ (opt_dateTimeSymbols || goog.i18n.DateTimeSymbols);
  ...
  formatYearOfWeek_(count, date) {
    var value = goog.date.getYearOfWeek(date.getFullYear(), date.getMonth(), date.getDate(), this.dateTimeSymbols_.FIRSTWEEKCUTOFFDAY,
   ...
   * @private
   */
  formatFractionalSeconds_(count, date) {
    ...
    return this.localizeNumbers_(
        value.toFixed(Math.min(3, count)).substr(2) + (count > 3 ? goog.string.padNumber(0, count - 3) : ''));
  }
   ...
   * @private
   */
  formatStandaloneDay_(count, date) {
    var value = date.getDay();
    switch (count) {
      case 5:
   ...
   * @private
   */
  formatStandaloneMonth_(count, date) {
    var value = date.getMonth();
    switch (count) {
      case 5:
  ...
  formatWeekOfYear_(count, date) {
    var weekNum = goog.date.getWeekNumber(date.getFullYear(), date.getMonth(), date.getDate(), this.dateTimeSymbols_.FIRSTWEEKCUTOFFDAY,
        this.dateTimeSymbols_.FIRSTDAYOFWEEK);
   ...
   * @private
   */
  formatTimeZoneRFC_(count, date, opt_timeZone) {
   ...
   * @private
   */
  formatTimeZone_(count, date, opt_timeZone) {
    opt_timeZone = opt_timeZone || goog.i18n.TimeZone.createTimeZone(date.getTimezoneOffset());
   ...
   * @private
   */
  formatTimeZoneId_(date, opt_timeZone) {
    opt_timeZone = opt_timeZone || goog.i18n.TimeZone.createTimeZone(date.getTimezoneOffset());
   ...
   * @private
   */
  formatTimeZoneLocationId_(count, date, opt_timeZone) {
    opt_timeZone = opt_timeZone || goog.i18n.TimeZone.createTimeZone(date.getTimezoneOffset());
   ...
   * @private
   */
  formatField_(patternStr, date, dateForDate, dateForTime, opt_timeZone) {
   ...
   * @return {string} localized string, potentially using native digits.
   */
  static localizeNumbers(input, opt_dateTimeSymbols) {
    input = String(input);
    ...
    for (var i = 0; i < input.length; i++) {
      ...
      parts.push((0x30 <= c && c <= 0x39) ?  // '0' <= c <= '9'
          String.fromCharCode(dateTimeSymbols.ZERODIGIT + c - 0x30) : input.charAt(i));
    }
    return parts.join('');
  }
