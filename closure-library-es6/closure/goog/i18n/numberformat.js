...
goog.i18n.NumberFormat = class {
   ...
   *     and minus sign.
   */
  constructor(pattern, opt_currency, opt_currencyStyle, opt_symbols) {
   ...
   * @return {!goog.i18n.NumberFormat} Reference to this NumberFormat object.
   */
  setShowTrailingZeros(showTrailingZeros) {
   ...
   * @return {!goog.i18n.NumberFormat} Reference to this NumberFormat object.
   */
  setBaseFormatting(baseFormattingNumber) {
  ...
  roundNumber_(number) {
    ...
    var shiftedNumber = this.significantDigits_ <= 0 ? Math.round(number * power) : Math.round(
  ...
  formatNumberGroupingNonRepeatingDigitsParts_(parts, zeroCode, intPart, groupingArray) {
    ...
    for (currentGroupSizeIndex = groupingArray.length - 1;
      ...
      for (var rightDigitIndex = 0; rightDigitIndex < currentGroupSize && ((digitLenLeft - rightDigitIndex - 1) >= 0);
           rightDigitIndex++) {
        rightToLeftParts.push(
            String.fromCharCode(zeroCode + Number(intPart.charAt(digitLenLeft - rightDigitIndex - 1)) * 1));
      }
   ...
   * @private
   */
  subformatFixed_(number, minIntDigits, parts) {
   ...
   * @private
   */
  subformatExponential_(number, parts) {
    if (number == 0.0) {
  ...
  getUnitFor_(base, plurality) {
    ...
    if (base < 3) {
      ...
      return {
        prefix: parts[1], suffix: parts[3], divisorBase: (previousNonNullBase + 1) - (parts[2].length - 1)
      };
    }
  }
   ...
   * @private
   */
  getUnitAfterRounding_(formattingNumber, pluralityNumber) {
    if (this.compactStyle_ == goog.i18n.NumberFormat.CompactStyle.NONE) {
      return goog.i18n.NumberFormat.NULL_UNIT_;
    }
   ...
   * @private
   */
  roundToSignificantDigits_(number, significantDigits, scale) {
