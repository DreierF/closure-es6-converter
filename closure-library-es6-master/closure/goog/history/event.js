...
/**
   ...
   *
   *
   , such as forward or back, clicking on a link, editing the URL, or
   {@code window.history.(go|back|forward)}.
   `setToken` or
   *     `replaceToken` call.
   * 
   * 
   * @final
   */
goog.history.Event = class extends goog.events.Event {
  /**
  * Event object dispatched after the history state has changed.
  ...
  *
  ...
  constructor(token, isNavigation) {
    super( goog.history.EventType.NAVIGATE);
