...
/**
   ...
   * 
   */
goog.format.EmailAddress = class {
  /**
  * Formats an email address string for display, and allows for extraction of
  * the individual components of the address.
  ...
  *
   */
  constructor(opt_address, opt_name) {
    /**
   ...
   * @private
   */
  static isQuoteNeeded_(name, specialChars) {
   ...
   */
  static isValidLocalPartSpec(str) {
   ...
   */
  static isValidDomainPartSpec(str) {
   ...
   * @protected
   */
  static parseInternal(addr, ctor) {
   ...
   * @protected
   */
  static parseListInternal(str, parser, separatorChecker) {
    var result = [];
    var email = '';
    var token;
   ...
   * @private
   */
  static isEscapedDlQuote_(str, pos) {
    if (str.charAt(pos) != '"') {
      return false;
    }
    var slashCount = 0;
