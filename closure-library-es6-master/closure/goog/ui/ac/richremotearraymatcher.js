...
/**
   ...
   *
   'token' query param.
   *
   'use_similar' query param which is
   "1" (opt_noSimilar==false) or "0" (opt_noSimilar==true).
   * 
   * 
   */
goog.ui.ac.RichRemoteArrayMatcher = class extends goog.ui.ac.RemoteArrayMatcher {
  /**
  * An array matcher that requests rich matches via ajax and converts them into
  * rich rows.
  *
  ...
  *
  ...
  requestMatchingRows(token, maxMatches, matchHandler) {
    ...
    super.requestMatchingRows(token, maxMatches, myMatchHandler);
  }
};
