...
/**
   ...
   *
   */
goog.ui.DatePicker = class extends goog.ui.Component {
  /**
  * DatePicker widget. Allows a single date to be selected from a calendar like
  * view.
  *
  ...
  *
   */
  constructor(opt_date, opt_dateTimeSymbols, opt_domHelper, opt_renderer) {
    super( opt_domHelper);
     ...
     * @private
     */
    this.symbols_ = /** @type {!goog.i18n.DateTimeSymbolsType} */ (opt_dateTimeSymbols || goog.i18n.DateTimeSymbols);
     ...
     * @private
     */
    this.renderer_ = opt_renderer || new goog.ui.DefaultDatePickerRenderer(this.getBaseCssClass(), this.getDomHelper());
     ...
     * @private
     */
    this.userSelectableDateRange_ = goog.date.DateRange.allTime();
    /**
 ...
 * @type {?Element}
     ...
     * @private
     */
    this.cellIdGenerator_ = goog.ui.IdGenerator.getInstance();
  }
  ...
  setUseNarrowWeekdayNames(b) {
    this.wdayNames_ = b ? this.symbols_.STANDALONENARROWWEEKDAYS : this.symbols_.STANDALONESHORTWEEKDAYS;
    this.redrawWeekdays_();
  }
  ...
  setDate_(date, fireSelection) {
    ...
    var sameMonth = date == this.date_ || date && this.date_ && date.getFullYear() == this.date_.getFullYear() && date.getMonth() == this.date_.getMonth();
 ...
 * @suppress {strictMissingProperties} Part of the go/strict_warnings_migration
  ...
  for (var y = 0; y < goog.ui.DatePicker.MAX_NUM_WEEKS_; y++) {
      ...
      for (var x = 0; x < 7; x++) {
        ...
        if (this.showOtherMonths_ || o.getMonth() == month) {
          ...
          var formattedDate = this.longDateFormat_ ? this.i18nDateFormatterDay2_.format(o) : this.i18nDateFormatterDay_.format(o);
    ...
    // are always shown as no month has less than 28 days).
      if (y >= 4) {
        var parentEl = /** @type {Element} */ (this.elTable_[y + 1][0].parentElement || this.elTable_[y + 1][0].parentNode);
  ...
  redrawWeekdays_() {
    ...
    var parentEl = /** @type {Element} */ (this.elTable_[0][0].parentElement || this.elTable_[0][0].parentNode);
...
/**
   ...
   *
   */
goog.ui.DatePickerEvent = class extends goog.events.Event {
