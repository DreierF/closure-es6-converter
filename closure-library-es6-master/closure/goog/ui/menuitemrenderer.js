...
goog.ui.MenuItemRenderer = class extends goog.ui.ControlRenderer {
  /**
   ...
   */
  createDom(item) {
    var element = item.getDomHelper().createDom(goog.dom.TagName.DIV, this.getClassNames(item).join(' '),
  ...
  hasCheckBoxStructure(element) {
    ...
    if (contentElement) {
      ...
      return !!child && goog.dom.isElement(child) && goog.dom.classlist.contains(/** @type {!Element} */ (child), checkboxClassName);
    }
    return false;
  }
  ...
  setEnableCheckBoxStructure(item, element, enable) {
    ...
    if (enable != this.hasCheckBoxStructure(element)) {
      ...
      if (enable) {
        ...
        var checkboxClassName = this.getCompositeCssClass_(goog.ui.MenuItemRenderer.CompositeCssClassIndex_.CHECKBOX);
        contentElement.insertBefore(item.getDomHelper().createDom(goog.dom.TagName.DIV, checkboxClassName),
