...
goog.ui.Zippy = class extends goog.events.EventTarget {
   ...
   */
  constructor(header, opt_content, opt_expanded, opt_expandedHeader, opt_domHelper, opt_role) {
    super();
     ...
     * @private
     */
    this.elContent_ = this.lazyCreateFunc_ || !opt_content ? null : this.dom_.getElement(/** @type {!Element} */ (opt_content));
    ...
    if (!goog.isDef(opt_expanded) && !this.lazyCreateFunc_) {
      ...
      if (this.elExpandedHeader_) {
        ...
        this.expanded_ = goog.dom.classlist.contains(this.elHeader_, goog.getCssName('goog-zippy-expanded'));
      }
    }
  ...
  getVisibleHeaderElement() {
    ...
    return expandedHeader && goog.style.isElementShown(expandedHeader) ? expandedHeader : this.elHeader_;
  }
  ...
  enableKeyboardEventsHandling_(header) {
    if (header) {
      ...
      this.keyboardEventHandler_.listen(this.keyHandler_, goog.events.KeyHandler.EventType.KEY, this.onHeaderKeyDown_);
    }
  }
...
/**
   ...
   * @final
   */
goog.ui.ZippyEvent = class extends goog.events.Event {
  /**
  * Object representing a zippy toggle event.
  *
